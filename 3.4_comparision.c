...
A11[i][j] = A[i][j];
A12[i][j] = A[i][j + newSize]; A21[i][j] = A[i
+ newSize][j]; A22[i][j] = A[i + newSize][j +
newSize];
B11[i][j] = B[i][j];
B12[i][j] = B[i][j + newSize]; B21[i][j] = B[i
+ newSize][j]; B22[i][j] = B[i + newSize][j +
newSize]; }
}
...
int **P2 = allocate_matrix(new_n); int
**P3 = allocate_matrix(new_n); int
**P4 = allocate_matrix(new_n); int
**P5 = allocate_matrix(new_n); int
**P6 = allocate_matrix(new_n); int
**P7 = allocate_matrix(new_n); int
**temp1 = allocate_matrix(new_n);
int **temp2 = allocate_matrix(new_n);
for (int i = 0; i < new_n; i++) {
for (int j = 0; j < new_n; j++) {
A11[i][j] = A[i][j];
A12[i][j] = A[i][j + new_n]; A21[i][j] =
A[i + new_n][j]; A22[i][j] = A[i + new_n][j
+ new_n]; B11[i][j] = B[i][j];
B12[i][j] = B[i][j + new_n]; B21[i][j] = B[i
+ new_n][j]; B22[i][j] = B[i + new_n][j +
new_n]; }
}
...
free_matrix(new_n, A11); free_matrix(new_n, A12); free_matrix(new_n, A21);
free_matrix(new_n, A22);
free_matrix(new_n, B11); free_matrix(new_n, B12); free_matrix(new_n, B21);
free_matrix(new_n, B22);
free_matrix(new_n, C11); free_matrix(new_n, C12); free_matrix(new_n, C21);
free_matrix(new_n, C22);
free_matrix(new_n, P1); free_matrix(new_n, P2); free_matrix(new_n, P3);
free_matrix(new_n, P4);
